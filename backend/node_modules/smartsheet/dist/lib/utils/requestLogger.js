...
function create(logger) {
    ...
    const logErrorResponse = (verb, requestOptions, error) => {
        ...
        logger.error('Response: Failure (HTTP %d)\n\tError Code: %d - %s\n\tRef ID: %s', error.statusCode, error.errorCode, error.message, error.refId);
    ...
    const logResponsePayload = (context, payload) => {
        if (underscore_1.default.isEmpty(payload))
            return;
    ...
    const logPreviewAndFullPayload = (context, payloadStr) => {
        if (underscore_1.default.isEmpty(payloadStr))
            return;
    ...
    const buildLoggingUrl = (requestOptions) => {
        ...
        if (!underscore_1.default.isEmpty(qs)) {
            qs = censorQueryParams(qs);
            queryParams =
                '?' +
                    underscore_1.default.pairs(qs)
    ...
    const censor = (s) => {
        if (underscore_1.default.isEmpty(s))
            return s;
    ...
    const buildCensor = function (blacklist) {
        return (obj) => underscore_1.default.mapObject(obj, (val, key) => {
